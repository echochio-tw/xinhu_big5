...
class inputChajian extends Chajian
{
	...
	*/
	public function getfieldcont($fid, $objs=null, $leox='', $iszb=0, $deval=false)
	{
		...
		if($fid=='刪'){
			$str='<a href="javascript:;" onclick="c.delrow(this,'.$xu0.')">刪</a>';
		}
		if($fid=='新增'){
		...
		if(isempt($val))$val='';
		if($deval !== false)$val = $deval; //設置默認值
		if(isempt($attr))$attr='';
		if($val!='' && contain($val,'{')){
		...
		if($type=='changeusercheck'||$type=='changeuser'||$type=='changedept'||$type=='changedeptcheck'||$type=='changedeptusercheck'){
			...
			$str   .= '<td nowrap><a href="javascript:;" onclick="js.changeclear(\'change'.$fname.'\')" class="webbtn">×</a><a href="javascript:;" id="btnchange_'.$fname.'" onclick="js.changeuser(\'change'.$fname.'\',\''.$type.'\')" class="webbtn">選擇</a></td></tr></table>';
		}
		if($type=='selectdatafalse' || $type=='selectdatatrue'){
			...
			$str   .= '<td nowrap><a href="javascript:;" onclick="c.selectdata(\''.$data.'\','.substr($type,10).',\''.$fname.'\',\''.$fieldname.'\','.$iszb.')" class="webbtn">選擇</a></td></tr></table>';
		}
		if($type=='htmlediter'){
	...
	public function getdatastore($type, $objs, $datanum, $fid='')
	{
		...
		//判斷是不是SQL([SQL] name,value from [Q]abc)
