...
class modeClassModel extends Model
{
	...
	public function getmoderows($uid, $sww='')
	{
		...
		$arr 	= $this->getall("`status`=1 and `type`<>'系統' $sww $where",'`id`,`num`,`name`,`table`,`type`,`isflow`,`isscl`','`sort`');
		return $arr;
	}
	...
	public function createlistpage($modeid, $lxss=0)
	{
		...
		$showzt	= false;
		if($isflow==1){
			$columnsstr = '{text:"申請人",dataIndex:"base_name",sortable:true},{text:"申請人部門",dataIndex:"base_deptname",sortable:true},{text:"單號",dataIndex:"sericnum"},';
		}
		$farr[] = array('name'=>'申請人','fields'=>'base_name');
		$farr[] = array('name'=>'申請人部門','fields'=>'base_deptname');
		$farr[] = array('name'=>'單號','fields'=>'sericnum');
		...
		if($isflow==1){
			$columnsstr.='{text:"狀態",dataIndex:"statustext"},';
		}
		$jgpstr 	= '<!--SCRIPTend-->';
		$hstart 	= '<!--HTMLstart-->';
		$hendts 	= '<!--HTMLend-->';
		$oldcont 	= @file_get_contents($path);
		$autoquye	= $this->rock->getcai($oldcont,'//[自定義區域start]','//[自定義區域end]');
		
		$isdaoru 	= m('flow_element')->rows("`mid`='$modeid' and `isdr`=1");
		$drstrbtn	= '';
		if($isdaoru>0){
			$drstrbtn	= "<span style=\"display:none\" id=\"daoruspan_{rand}\"><button class=\"btn btn-default\" click=\"daoru,1\" type=\"button\">導入</button>&nbsp;&nbsp;&nbsp;</span>";
		}
		...
		if($showzt){
			...
			$zthtml = '<td style="padding-left:10px"><select class="form-control" style="width:120px" id="selstatus_{rand}"><option value="">-全部狀態-</option>';
		...
		$placeholder= '關鍵字';
		if($isflow==1)$placeholder= '關鍵字/申請人/單號';
...
<div>
	...
	<tr>
		...
		<td align=\"right\" id=\"tdright_{rand}\" nowrap>
			".$drstrbtn."<button class=\"btn btn-default\" click=\"daochu\" type=\"button\">導出 <i class=\"icon-angle-down\"></i></button> 
		</td>
	</tr>
	</table>
</div>
...
$(document).ready(function(){
	...
	var c = {
		...
		daoru:function(){
			...
			addtabs({num:'daoru".$num."',url:'flow,input,daoru,modenum=".$num."',icons:'plus',name:'導入".$name."'});
		},
