...
class loginClassModel extends Model
{
	...
	public function start($user, $pass, $cfrom='', $device='')
	{
		...
		$posts  = $user;
		if($posts=='管理員')return '不能使用管理員的名字登錄';
		...
		m('log')->addlog(''.$cfrom.'登錄', '['.$posts.']'.$loginx.''.$logins.'', array(
			'optid'		=> $uid, 
			'optname'	=> $name,
			'ip'		=> $ip,
			'web'		=> $web,
		...
		if($msg==''){
			...
			$this->delete("`moddt`<'$lastd'"); //刪除2天前記錄
	...
	public function autologin($aid=0, $token='', $ism=0)
	{
		...
		if($baid==0){
			$uid 	= (int)$this->rock->cookie('mo_adminid','0');//用cookie登錄
