...
class adminClassAction extends Action
{
	...
	public function beforeshow($table)
	{
		...
		if(isempt($opts)){
			$this->db->query($sql1);
			$this->option->setval('adminbug1',$this->now, '用戶bug1');
		}
		return array(
			'fields'=> $fields,
			'where'	=> $s
	...
	public function saveadminplAjax()
	{
		...
		foreach($rows as $k=>$rs){
			...
			if($db->rows("`user`='$user'")>0)$user = $user.'1'; //相同用戶名?
			...
			if($deptarr['deptid']==0)return returnerror('行'.($k+1).'找不到對應頂級部門['.$rs['deptname'].']');
		...
		return returnsuccess('成功導入'.$oi.'個用戶');
	}
