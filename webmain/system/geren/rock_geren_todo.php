...
$(document).ready(function(){
	var a = $('#veiw_{rand}').bootstable({
		...
		columns:[{
			text:'類型',dataIndex:'title',renderer:function(v,d){
				var s = v;
				if(d.status==1)s='<font color=#aaaaaa>'+v+'</font>';
				return s;
			}
		},{	
			text:'信息內容',align:'left',dataIndex:'mess',renderer:function(v,d, oi){
				var s = v;
				if(d.status==1)s='<font color=#aaaaaa>'+v+'</font>';
			...
			text:'時間',dataIndex:'optdt',renderer:function(v,d){
				var s = v;
				if(d.status==1)s='<font color=#aaaaaa>'+v+'</font>';
				return s;
			}
		},{
			text:'狀態',dataIndex:'status',renderer:function(v){
				var s = '<font color=red>未讀</font>';
				if(v==1)s='<font color=#aaaaaa>已讀</font>';
				return s;
			}
		}],
...
<ul class="floats">
	<li class="floats50">
		<button class="btn btn-success" click="yidu,1"  type="button">標為已讀</button>
	</li>
