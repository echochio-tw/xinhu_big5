	...
	*/
	...
	public function flowbillafter($table, $rows)
	{
		...
		if($this->atypess!='error'){
			...
			foreach($rows as $k=>$rs){
				$errorsm	= '';
				$chuli 		= '到[流程模塊→流程審核步驟]下對應的步驟設置審核人';
				$errtype	= 0;//有步驟沒審核人
				if(isempt($rs['nowcheckid'])){
					if($rs['nowcourseid']=='0'){
						$errorsm = '<font color=blue>當前沒有審核步驟</font>';
						$chuli 		= '到[流程模塊→流程單據查看]刪除最後一條處理記錄，然後[重新匹配流程]';
						$errtype	= 1; //沒有步驟
					}else{
						$errorsm = '<font color=red>當前沒有審核人</font>';
					}
				}else{
					$errorsm = '<font color=#800000>審核人帳號已停用</font>';
					$errtype	= 2; //人員停用
				}
	...
	public function flowtodosafter($table, $rows)
	{
		...
		if($rows){
			foreach($rows as $k=>$rs){
				...
				if($rs['isread']=='1'){
					$rows[$k]['ishui']  = 1;
					$rows[$k]['isread'] = '<font color="#888888">已讀</font>';
				}else{
					$rows[$k]['isread'] = '<font color="red">未讀</font>';
				}
			}
		}
		return array(
