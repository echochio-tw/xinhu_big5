...
$(document).ready(function(){
	...
	var c = {
		...
		plliang:function(){
			...
			if(d.length<=0){
				js.msg('msg','請先用復選框選中行');
				return;
			}
			this.checkd = d;
			js.prompt('批量處理同意','請輸入批量處理同意說明(選填)',function(lxbd,msg){
				if(lxbd=='yes'){
		...
		plliangsos:function(oi){
			...
			$('#plchushumm').html('批量處理中('+len+'/'+(oi+1)+')...');
			if(oi>=len){
				$('#plchushumm').html('處理完成，成功<font color=green>'+this.cgshu+'</font>條，失敗<font color=red>'+this.sbshu+'</font>條');
				this.reload();
				this.plbool=false;
				return;
			}
			...
			js.ajax(js.getajaxurl('check','flowopt','flow'),cns, function(ret){
				if(ret.success){
					...
					c.sbshu++;
					js.msg('msg','['+d.modename+']'+ret.msg+'，不能使用批量來處理，請打開詳情去處理。');
				}
				c.plliangsos(oi+1);
			},'post,json');
		}
...
<div>
	<table width="100%">
	<tr>
	<td style="padding-right:10px;" id="tdleft_{rand}" nowrap><button class="btn btn-primary" click="plliang,0" type="button">批量處理同意</button></td>
	<td nowrap>
		<select style="width:150px" id="mode_{rand}" class="form-control" ><option value="0">-選擇模塊-</option></select>	
	</td>
	<td  style="padding-left:10px">
		<div style="width:130px"  class="input-group">
			<input placeholder="申請日期" readonly class="form-control" id="dt1_{rand}" >
	...
	<td  style="padding-left:10px">
		<input class="form-control" style="width:160px" id="key_{rand}"   placeholder="姓名/部門/單號">
	</td>
	...
	<td  width="80%" style="padding-left:10px">
		...
		<button class="btn btn-default" id="state{rand}_" click="changlx," type="button">全部狀態</button>
		<button class="btn btn-default" id="state{rand}_0" click="changlx,0" type="button">待審核</button>
		<button class="btn btn-default" id="state{rand}_1" style="color:green" click="changlx,1" type="button">已審核</button>
		<button class="btn btn-default" id="state{rand}_2" style="color:red" click="changlx,2" type="button">未通過</button>
		<button class="btn btn-default" id="state{rand}_5" style="color:#888888" click="changlx,5" type="button">已作廢</button>
