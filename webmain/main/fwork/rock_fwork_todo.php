...
$(document).ready(function(){
	...
	var a = $('#view_{rand}').bootstable({
		...
		columns:[{
			...
			text:'提醒時間',dataIndex:'adddt',sortable:true
		},{
			text:'摘要',dataIndex:'summary',align:'left',width:300
		},{
			text:'操作人',dataIndex:'optname'
		},{
			text:'單據操作時間',dataIndex:'optdt'
		},{
			text:'狀態',dataIndex:'isread',sortable:true
		}],
		...
		load:function(a){
			if(!bools){
				var s = '<option value="">-選擇模塊-</option>',len=a.flowarr.length,i,csd,types='';
				for(i=0;i<len;i++){
					csd = a.flowarr[i];
					if(types!=csd.type){
						if(types!='')s+='</optgroup>';
	...
	var c = {
		...
		setss:function(){
			addtabs({name:'單據提醒設置',num:'searchremind',url:'flow,page,remind,atype=my',icons:'cog'});
		},
		del:function(){
...
<div>
	...
	<td  style="padding-right:10px">
		<button class="btn btn-default" click="setss" type="button"><i class="icon-cog"></i> 單據提醒設置</button>
	</td>
	<td nowrap>
		<select style="width:150px" id="mode_{rand}" class="form-control" ><option value="">-所有模塊-</option></select>	
	</td>
	...
	<td align="right" nowrap>
		<button class="btn btn-default" id="xiang_{rand}" click="view" disabled type="button">詳情</button> &nbsp; <button class="btn btn-default" click="daochu,1" type="button">導出</button>&nbsp; 
