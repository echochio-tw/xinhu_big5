...
$(document).ready(function(){
	...
	var a = $('#view_{rand}').bootstable({
		...
		columns:[{
			...
			text:'對應字段',dataIndex:'fields'
		},{
			text:'分類',dataIndex:'iszb',renderer:function(v,d){
				var s='<font color=#ff6600>主表</font>';
				if(v>0)s='第'+d.iszb+'個子表';
				return s;
			}
		},{
			text:'元素類型,<a target="_blank" href="<?=URLY?>view_element.html">說明</a>',dataIndex:'fieldstype'
		},{
			text:'默認值',dataIndex:'dev',editor:true
		},{
			text:'排序號',dataIndex:'sort',editor:true,sortable:true
		},{
			text:'錄入列',dataIndex:'islu',type:'checkbox',editor:true,sortable:true
		},{
			...
			text:'可統計',dataIndex:'istj',type:'checkbox',editor:true,sortable:true
		},{
			text:'唯一值',dataIndex:'isonly',type:'checkbox',editor:true,sortable:true
		},{
			text:'可導入',dataIndex:'isdr',type:'checkbox',editor:true,sortable:true
		},{
			text:'對齊',dataIndex:'isalign',type:'select',editor:true,sortable:true,renderer:function(v,d){
...
<div>
	...
	<tr>
	<td align="left">
		<select id="mode_{rand}" style="width:180px" class="form-control" ><option value="0">-選擇模塊-</option></select>
	</td>
	...
	<td width="80%" align="left" nowrap style="padding-left:10px;">
		<div class="btn-group">
		<button class="btn btn-default" id="luc_{rand}" disabled click="inputs,0" type="button">PC端錄入頁布局</button>
		<button class="btn btn-default" id="luz_{rand}" disabled click="zhanshi,0" type="button">PC端展示</button>
		<button class="btn btn-default" id="luzz_{rand}" disabled click="zhanshi,1" type="button">手機展示</button>
		<button class="btn btn-default" id="lu_{rand}" disabled click="lulu,0" type="button">PC錄入頁</button>
		<button class="btn btn-default" id="lum_{rand}" disabled click="lulu,1" type="button">手機錄入頁</button>
		<button class="btn btn-default" id="changelieb_{rand}" disabled click="changelieb" type="button">生成列表頁</button>
		</div>
	</td>
	<td align="right" nowrap>
		<button class="btn btn-default" click="rexuhao" type="button">刷新序號</button>&nbsp; 
		<button class="btn btn-warning" id="add_{rand}" disabled click="clickwin,0" type="button">新增</button>&nbsp; 
		<button class="btn btn-info" id="edit_{rand}" click="clickwin,1" disabled type="button">編輯</button>&nbsp; 
		<button class="btn btn-danger" click="del" disabled id="del_{rand}" type="button">刪除</button>
	</td>
	</tr>
	</table>
...
<div class="tishi">
1、PC端錄入頁布局：設置電腦PC端錄入頁面的布局的。<br>
2、PC端展示：設置電腦PC端單據詳情頁面展示的樣子。<br>
3、手機展示：設置手機上單據詳情頁面展示的樣子。<br>
</div>
