...
$(document).ready(function(){
	...
	var at = $('#optionview_{rand}').bootstable({
		...
		columns:[{
			...
			text:'步驟數',dataIndex:'shu'
		},{
			text:'ID',dataIndex:'id'
		}],
		itemdblclick:function(ad,oi,e){
			$('#downshow_{rand}').html('設置<b>['+ad.id+'.'+ad.name+']</b>的流程管理');
			setid=ad.id;
	...
	var a = $('#view_{rand}').bootstable({
		...
		columns:[{
			...
			text:'適用于',dataIndex:'recename'
		},{
			text:'分支條件',dataIndex:'whereid'
		},{
			text:'編號',dataIndex:'num'
		},{
			text:'審核人類型',dataIndex:'checktype'
		},{
			text:'審核人',dataIndex:'checktypename'
		},{
			text:'排序號',dataIndex:'sort',editor:true
		},{
			text:'狀態',dataIndex:'status',type:'checkbox',editor:true
		},{
			text:'下級步驟',dataIndex:'nid',renderer:function(v){
				if(v=='0')v='&nbsp;';
				return v;
			}
		},{
			...
			text:'',dataIndex:'opts',align:'left',renderer:function(v,d,i){
				...
				if(d.id>0){
					...
					if(d.stotal==0)s+=' <a onclick="optess{rand}('+i+',1)" href="javascript:;">刪</a>';
				}
				return s;
			}
		}],
	...
	var c = {
		...
		addflow:function(mid, id,tex){
			...
			var icon='plus',name='新增['+tex+']下的步驟';
			if(id>0){
				icon='edit';
				name='編輯['+tex+']步驟';
			}
		...
		showflow:function(ret, lef,mid,lev){
			...
			for(i=0;i<len;i++){
				...
				oi++;
				l = ks;
				if(d.childshu>1)jg=150; //有下級間隔大點
				if(i>0){
					l+=jg;
				}
				t = 120*d.level;
		...
		jian:function(fid,tid){
			...
			l  = fleft+fo.width()*0.5-8; //箭頭開始位置
			t  = parseInt(fo.css('top'))+fo.height()+5; //箭頭結束位置
			
			
			
			jw = tleft - fleft - fo.width() + to.width()*0.5;//兩個的位置寬
			...
			dw = Math.sqrt(jw*jw+jh*jh); //對角線長度(勾股定理)
			
			
			var jds = Math.atan(jw/jh)*180/Math.PI; //旋轉角度
		...
		righmenu:function(o1,e){
			...
			var ba = courseobj[id];
			d.push({'name':'新增下級步驟',text:ba.name,mid:0,'id':id,lx:0});
			if(id>0){
				d.push({'name':'編輯',text:ba.name,'id':id,lx:1,mid:ba.mid});
				if(ba.childshu==0)d.push({'name':'<font color=red>刪除</font>','id':id,lx:2});
			}
			this.robj.setData(d);
		...
		rightclick:function(d){
			...
			if(lx==1)this.addflow(d.mid, d.id,d.text); //編輯
			if(lx==2)this.delflow(d.id);
		},
		delflow:function(id){
			js.confirm('確定要刪除此步驟嗎？',function(jg){
...
<td width="25%">
	<div class="panel panel-info" style="margin:0px">
	  <div class="panel-heading">
		<h3 class="panel-title">流程模塊(雙擊顯示步驟)</h3>
	  </div>
...
<td>
	<div>
		...
		<td align="right" nowrap>
			<button class="btn btn-primary" click="adddowns" id="down_{rand}" disabled type="button">新增下級步驟</button>
		</td>
		</tr>
		</table>
