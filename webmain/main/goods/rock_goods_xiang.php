<?php if(!defined('HOST'))die('not access');?>
<script >
$(document).ready(function(){
	
	var at = $('#optionview_{rand}').bootstree({
		url:js.getajaxurl('gettreedata','option','system',{'num':'goodstype'}),
		columns:[{
			text:'物品分類',dataIndex:'name',align:'left',xtype:'treecolumn',width:'79%'
		},{
			text:'ID',dataIndex:'id',width:'20%'
		}],
		load:function(d){
			c.loadfile('0','所有出入庫詳情');
		},
		itemdblclick:function(d){
			c.loadfile(d.id,d.name);
		}
	});
	
	var a = $('#view_{rand}').bootstable({
		tablename:'goodss',autoLoad:false,celleditor:true,fanye:true,dir:'desc',sort:'a.id',checked:true,
		url:publicstore('{mode}','{dir}'),storebeforeaction:'xiangbeforeshow',storeafteraction:'xiangaftershow',
		columns:[{
			text:'名稱',dataIndex:'name',align:'left'
		},{
			text:'分類',dataIndex:'typeid',align:'left'
		},{
			text:'類型',dataIndex:'kind',sortable:true
		},{
			text:'日期',dataIndex:'applydt',sortable:true
		},{
			text:'操作人',dataIndex:'optname'
		},{
			text:'數量',dataIndex:'count',sortable:true,align:'right'
		},{
			text:'說明',dataIndex:'explain',align:'left'
		},{
			text:'狀態',dataIndex:'status'
		}]
	});
	var c = {
		search:function(){
			var s=get('key_{rand}').value;
			a.setparams({key:s,dt:get('dt1_{rand}').value},true);
		},
		clickdt:function(o1, lx){
			$(o1).rockdatepicker({initshow:true,view:'month',inputid:'dt'+lx+'_{rand}'});
		},
		daochu:function(){
			a.exceldown('物品出入庫詳情');
		},
		loadfile:function(spd,nsd){
			$('#megss{rand}').html(nsd);
			a.setparams({'typeid':spd}, true);
		},
		genmu:function(){
			this.loadfile('0','所有出入庫詳情');
		},
		del:function(){
			a.del({checked:true,url:js.getajaxurl('delxiang','{mode}','{dir}')});
		}
	};
	

	js.initbtn(c);
	
	$('#optionview_{rand}').css('height',''+(viewheight-25)+'px');
});
</script>

<table width="100%">
<tr valign="top">
<td width="220">
	<div style="border:1px #cccccc solid">
	  <div id="optionview_{rand}" style="height:400px;overflow:auto;"></div>
	</div>  
</td>
<td width="10"></td>
<td>


<div>
<table width="100%"><tr>
	<td align="left" nowrap>
			<button class="btn btn-default" click="genmu"  type="button">所有詳情</button>&nbsp; 
	</td>
	<td nowrap  style="padding-left:10px">
		<div style="width:120px"  class="input-group">
			<input placeholder="月份" readonly class="form-control" id="dt1_{rand}" >
			<span class="input-group-btn">
				<button class="btn btn-default" click="clickdt,1" type="button"><i class="icon-calendar"></i></button>
			</span>
		</div>
	</td>

	<td style="padding-left:10px">
	<input class="form-control" style="width:200px" id="key_{rand}"   placeholder="物品名/操作人">
	</td>
	<td style="padding-left:10px">
		<button class="btn btn-default" click="search" type="button">搜索</button> 
	</td>
	
	<td width="90%">
		&nbsp;&nbsp;<span id="megss{rand}"></span>
	</td>
	<td align="right" nowrap>
		<button class="btn btn-default" click="daochu,1" type="button">導出</button>&nbsp;&nbsp;
		<button class="btn btn-danger" id="del_{rand}" click="del" type="button"><i class="icon-trash"></i> 刪除</button>
	</td>
</tr></table>
</div>
<div class="blank10"></div>
<div id="view_{rand}"></div>

</td>
</tr>
</table>