...
$(document).ready(function(){
	var optdt = '',loadci=0, taskarr={}, miao=200; //定時秒數
	var c= {
		...
		gettotalshow:function(a){
			...
			if(s!=''){
				...
				if(tx=='0'){
					$('#tishidivshow').fadeIn();
					$('#tishicontent').html(s);
					notifyobj.showpopup(s1,{icon:'images/todo.png',rand:'systodo',title:'系統提醒',click:function(b){
						opentixiangs();
						return true;
					}});
				}
	...
	openmobile=function(){
		js.tanbody('loginmobile','登錄手機版', 300,200,{
			html:'<div  style="height:160px;padding:5px" align="center"><div><img id="logeweerew" src="images/logo.png" width="130" height="130"></div><div>直接掃一掃即可登錄</div></div>'
		});
		var surl = js.getajaxurl('getqrcode','index','home'),surls = js.getajaxurl('getqrcores','index','home');
		$.get(surls,function(ass){
			if(ass!='ok'){
				$('#logeweerew').parent().html('<div style="padding:10px 20px;text-align:left">未開啟gd庫，不能生成二維碼，<br>可手機瀏覽器輸入地址:<br>'+ass+'</div>');
			}else{
	...
	openreim=function(){
		...
		if(!ops){
			js.cliendsend('focus',{},false,function(){
				js.confirm('可能沒有使用REIM的PC客戶端，是否打開網頁版的？',function(jg){
					if(jg=='yes'){
