...
class Action extends mainAction
{
	...
	{
		if($this->loginsubscribe())return;//免驗證
		...
		if($uid==0){
			...
			if(!isajax())$this->rock->location($lurl); //不是ajax跳轉到登錄頁面
			echo '沒有登錄，去<a href="'.$lurl.'">[登錄]</a>';
			exit();
		}
	}
	...
	*/
	public function publicsaveAjax()
	{
		...
		$id		= (int)$this->post('id');
		$oldrs  = false;
		if(isempt($table))return returnerror('錯誤表名');
		if(!$this->checksignature($this->post('tablename_postabc')))return returnerror('無效請求');
		$db		= m($table);
		$where	= "`id`='$id'";
		if($id==0)$where='';
		...
		$editrecord			= $this->post('editrecord_postabc'); //是否保存修改記錄
		...
		if(isempt($submditfi))return returnerror('無效字段');
	...
	public function publicsavevalueAjax()
	{
		...
		if(!$this->checksignature($this->post('tablename')))return '無效請求';
	...
	public function exceldown($arr)
	{
		...
		$rows	= $arr['rows'];
		$exceltype	= $this->post('exceltype','xls'); //保存文件類型
		$headArr	= array();
