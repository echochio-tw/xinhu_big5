function initbody(){
	...
	if(typeof(applicationCache)=='undefined'){
		js.msg('msg','您的瀏覽器太低了無法達到想要的預覽效果<br>建議使用IE10+，Firefox，Chrome等高級點的',60);
	}
	if(a.length<=1)$('.topmenubg').html('');
	var ddsata=[{
		name:'<i class="icon-lock"></i> 修改密碼',num:'pass',url:'system,geren,pass',names:'修改密碼'
	},{
	...
	$('#indexuserl').rockmenu({
		...
		itemsclick:function(d){
			if(d.num=='exit'){
				js.confirm('確定要退出系統嗎？',function(bn){
...
function showmenula(a){
	...
	if(s=='')s='<div style="padding:30px;color:#cccccc" align="center">暫無</div>';
	$('#menulist').html(s);
}
