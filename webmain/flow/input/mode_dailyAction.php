...
class mode_dailyClassAction extends inputAction{
	...
	protected function savebefore($table, $arr, $id, $addbo){
		...
		if($type==0 && $dt>$this->date)return '日期['.$dt.']還是個未來呢';
		$to 	= $this->mdb->rows($where);
		if($to>0)return '該類型日期['.$dt.']段已申請了';
	}
	...
	public function anxyfxaftershow($table, $rows)
	{
		...
		//0未寫,1已寫,2請假,3休息日,4沒入職或已離職,5不需要寫日報,時間還沒到
	...
	{
		return array(
			'ztdt' => $this->ztdt.',週'.c('date')->cnweek($this->ztdt).''
		);
	}
