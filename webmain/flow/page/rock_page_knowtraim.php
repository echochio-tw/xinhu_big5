...
$(document).ready(function(){
	{params}
	var modenum = 'knowtraim',modename='考試培訓',isflow=0,modeid='56',atype = params.atype,pnum=params.pnum;
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'考試培訓('+nowtabs.name+')',
				cont:'考試培訓('+nowtabs.name+')的列表的',
				explain:'訂閱[考試培訓]的列表',
				objtable:a
			});
		},
		...
		daoru:function(){
			...
			addtabs({num:'daoruknowtraim',url:'flow,input,daoru,modenum=knowtraim',icons:'plus',name:'導入考試培訓'});
		},
	...
	var bootparams = {
		...
		params:{atype:atype},
		columns:[{text:"標題",dataIndex:"title"},{text:"開始時間",dataIndex:"startdt",sortable:true},{text:"截止時間",dataIndex:"enddt"},{text:"單選題數",dataIndex:"dsshu"},{text:"多選題數",dataIndex:"dxshu"},{text:"考試對象",dataIndex:"recename"},{text:"培訓題庫",dataIndex:"tikuname"},{text:"培訓人數",dataIndex:"reshu"},{text:"已答數",dataIndex:"ydshu",sortable:true},{text:"總分",dataIndex:"zfenshu",sortable:true},{text:"合格分數",dataIndex:"hgfen"},{text:"說明",dataIndex:"explain"},{text:"狀態",dataIndex:"state",sortable:true},{
...
c.setcolumns('title',{
	renderer:function(v,d,oi){
		return ''+v+' <a onclick="tongji{rand}('+oi+')" href="javascript:;">[統計]</a>';
	}
});

tongji{rand}=function(oi){
	var d = a.getData(oi);
	addtabs({num:'kuntotog'+d.id+'',name:'統計['+d.title+']',url:'flow,page,kuntraimtotal,id='+d.id+''});
}
