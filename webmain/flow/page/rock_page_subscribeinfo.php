<?php
/**
*	模塊：subscribeinfo.訂閱報表，
*	說明：自定義區域內可寫您想要的代碼，模塊列表頁面，生成分為2塊
*	來源：流程模塊→表單元素管理→[模塊.訂閱報表]→生成列表頁
*/
...
$(document).ready(function(){
	{params}
	var modenum = 'subscribeinfo',modename='訂閱報表',isflow=0,modeid='68',atype = params.atype,pnum=params.pnum;
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'訂閱報表('+nowtabs.name+')',
				cont:'訂閱報表('+nowtabs.name+')的列表的',
				explain:'訂閱[訂閱報表]的列表',
				objtable:a
			});
		},
		...
		daoru:function(){
			...
			addtabs({num:'daorusubscribeinfo',url:'flow,input,daoru,modenum=subscribeinfo',icons:'plus',name:'導入訂閱報表'});
		},
	...
	var bootparams = {
		...
		params:{atype:atype},
		columns:[{text:"訂閱標題",dataIndex:"title"},{text:"訂閱內容",dataIndex:"cont"},{text:"添加時候",dataIndex:"optdt",sortable:true},{text:"訂閱提醒對象",dataIndex:"recename"},{text:"文件",dataIndex:"filepath"},{
...
$('#addbtn_{rand}').html('<i class="icon-cog"></i> 訂閱管理');
	c.clickwin=function(){
		addtabs({url:'flow,page,subscribe,atype=my',name:'我訂閱管理',num:'rssglmy','icons':'cog'});
	}
